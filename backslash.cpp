// backslash.cpp     ASCII art backslash as a C++ string2
// A C++ function that returns a string that, when printed on cout,3
// renders a backslash out of stars with a given width (width and height4
// must be equal).   5
// If width less than 2, the function should return an empty string.6
...
int main(int argc, char *argv[])
{
    int i;
	    int width;
			...
			// check the number of arguments24

			if (argc!=2) {
			    cerr << "Usage: " << argv[0] << " width" << endl;
									       exit(1);
											 }
											    // convert command line parameter and call function31

											    width=atoi(argv[1]);
														  cout << backslash(width) << flush; //function call to draw the backslash34
